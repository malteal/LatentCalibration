...
from .detector_coords import numpy_locals_to_mass_and_pt
...
class Substructure:
    ...
    def __init__(self, jet, R):
        ...
        if isinstance(jet, np.ndarray):
            ...
            rclst = pyjet.cluster(jet, R=R, p=1)
            jet = rclst.inclusive_jets()[0]
        self._cnsts = jet.constituents_array()
        self._cnsts = self._cnsts.view(dtype=np.float64).reshape(
            self._cnsts.shape + (-1,)
        )

        # rclst = pyjet.cluster(self._cnsts, R=R, p=1)
