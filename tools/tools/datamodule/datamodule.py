...
class MultiFileDataset(IterableDataset):
    """General class for handling multiple files
    
    takes in a processing_func that from the data_lst creates a pytorch dataloader
    
    it then iterates over all the dataloaders and returns the data
    """
    def __init__(self, data_lst:list, batch_size:int, processing_func):
