...
def prepare_mnist(df, dummy_val=-1, max_cnstits=None):
    ...
    return (np.float32(X), np.all(mask, -1), min_cnstits, max_cnstits, len(X),
            y)
...
def matrix_to_point_cloud(sample: Union[np.ndarray, T.Tensor],
                          idx_numbers: Union[np.ndarray, T.Tensor], num_per_event_max: int = None):
    """sample: n_features x pc
    idx_numbers: 0,0,0,1,1,2,3,3,... - same length as sample"""

    if isinstance(sample, T.Tensor):
        device= sample.device
        n_pc, num_per_event = T.unique(idx_numbers, return_counts=True)

        if num_per_event_max is None:
            num_per_event_max = num_per_event.max().item()
        elif num_per_event_max is not None:
            mask_max_cnts = num_per_event < num_per_event_max
            mask_sample = T.isin(idx_numbers, n_pc[mask_max_cnts])
            sample = sample[mask_sample.flatten()]
            num_per_event = num_per_event[mask_max_cnts]
            n_pc = n_pc[mask_max_cnts]

        n_events = len(n_pc)

        mask = T.arange(0, num_per_event_max, device=device).unsqueeze(0) < num_per_event.unsqueeze(1)

        padded_tens = T.ones((n_events, num_per_event_max, sample.shape[1]), device=device) * -1
    else:
