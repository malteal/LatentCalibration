...
class JetMappablePartial(Dataset, JetHDFBase):
    ...
    def load_epoch(self, epoch: int) -> None:
        ...
        log.info(f"Loaded {len(self)} jets from {self.files_per_epoch} files. Idx: [{start}, {end}]")
...
if __name__ == "__main__":
    ...
    from tools import hydra_utils, misc
    from src.datamodules.preprocessing import torch_log_squash, torch_locals_to_mass_and_pt
    import hydra
    import sys
    ...
    paths = misc.load_yaml('/home/users/a/algren/work/diffgae/configs/data/jetclass_base/mappartial.yaml')
    paths['path'] = '/srv/fast/share/rodem/JetClassH5/train_100M/'
    data = hydra.utils.instantiate(paths)()
    
    emb_id = torch.tensor(data.data_dict['csts_id'])[:10]
    
    emb_id = torch.nn.functional.one_hot(emb_id)
    
    emb = torch.nn.Embedding(8, 8)

    
    t = emb(emb_id.argmax(-1))
    pt_mass = []
    labels=[]
    for i in range(1):
    # for i in range(len(data.file_list)//data.files_per_epoch):
        data.load_epoch(i)
        inpt = np.stack(data.data_dict['inpt'],0)[..., [1,2,0]]
        mask = np.stack(data.data_dict['mask'])
        labels.append( np.stack( data.data_dict['labels']))
        pt = torch.tensor(inpt)
        pt_mass.append(torch_locals_to_mass_and_pt(pt,mask).numpy())

    pt_mass = np.concatenate(pt_mass, 0)
    labels = np.concatenate(labels, 0)



    
    plt.hist(pt_mass[:, 0], bins=100)
    
    plt.figure()
    for label in np.unique(labels):
        plt.hist(pt_mass[labels == label, 1], bins=100, alpha=0.5,
                 label=label, range=[-1, 10])
    plt.legend()
    # plt.hist(t.numpy()[:, 1], bins=100)

    # dataset = (
    # wds.WebDataset(paths)
    # .shuffle(1000)
    # # .decode("pil")
    ...
    # # dataloader = DataLoader(dataset_combine, batch_size=128, num_workers=8, pin_memory=True, shuffle=False, drop_last=False)
