...
from tools.tools.visualization.atlas_utils import pid_convertion
...
def read_jetclass_file(
    ...
    if particle_features is None:
        ...
        if pid_bool:
            particle_features += ["part_pid"]
        else:
            particle_features += ["part_isPhoton",
    ...
    with uproot.open(filepath) as f:
        ...
        if jet_labels[0] in tree.keys():
            labels = tree.arrays(expressions=jet_labels, library="pd")
        else:
            # remove label_
            label_str = [i.split('_')[-1] for i in jet_labels]

            # get idx value
            label=np.argmax([i in filepath.as_posix() for i in label_str])
            
            # extend to the length of the jets
            labels = np.ones(jets.shape[0], dtype=int) * label
