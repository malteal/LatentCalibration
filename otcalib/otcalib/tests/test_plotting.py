...
class TestingPlotting(unittest.TestCase):
    ...
    def testing_1d_plots(self):
        ...
        fig, fig_cern = uplot.plot_1d_hist(
            self.source, self.target, self.source, "Dummy name", "cern"
        )
        self.assertIsInstance(fig, plt.Figure)
        self.assertIsInstance(fig_cern, plt.Figure)
        fig, fig_cern = uplot.plot_1d_hist(
            self.source, self.target, self.source, "Dummy name", None
        )
        self.assertEqual(fig_cern, None)
